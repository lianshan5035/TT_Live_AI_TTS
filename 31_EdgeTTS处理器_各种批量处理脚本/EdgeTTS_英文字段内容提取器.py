...
class EnglishFieldExtractor:
    def __init__(self):
        ...
        print(f"🎵 EdgeTTS 英文字段内容提取器")
        print(f"📁 输入目录: {self.input_dir}")
        print(f"📁 输出目录: {self.output_dir}")
        print(f"🎯 专门提取'英文'字段的内容")
    ...
    async def process_excel_file(self, file_path, max_rows=10):
        ...
        try:
            ...
            for index in range(process_rows):
                ...
                output_file = os.path.join(self.output_dir, file_base, output_filename)
...
def main():
    ...
    processor = EnglishFieldExtractor()
    success = asyncio.run(processor.process_all_files())
    
    if success:
        print("\n🎉 英文字段内容提取完成!")
        print("💡 已成功提取'英文'字段内容并生成音频")
    else:
        print("\n❌ 英文字段内容提取失败!")
        print("💡 请检查配置和文件格式")

...
class ConservativeBatchProcessor:
    def __init__(self):
        ...
        print(f"🐌 EdgeTTS 保守批量处理器启动")
        print(f"📁 输入目录: {self.input_dir}")
        print(f"📁 输出目录: {self.output_dir}")
        print(f"🌐 API服务: {len(self.tts_urls)} 个")
        print(f"⏱️ 基础延迟: {self.base_delay}秒 (保守模式)")
    ...
    def generate_audio_with_retry(self, text, voice, emotion, output_file, max_retries=5):
        """带重试机制的音频生成 - 保守模式"""
        for attempt in range(max_retries):
            try:
                # 保守延迟
                if attempt > 0:
                    delay = self.current_delay + random.uniform(0, 5)
                    print(f"⏳ 重试前等待 {delay:.1f}秒...")
                    time.sleep(delay)
                else:
                    # 首次请求也延迟
                    delay = self.current_delay + random.uniform(0, 3)
    ...
    def process_excel_file(self, file_path):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                # 进度显示
                if (index + 1) % 5 == 0:  # 每5个文件显示一次进度
                    print(f"📊 进度: {index + 1}/{total_rows} ({success_count} 成功, {error_count} 失败)")
                    print(f"⏱️ 当前延迟: {self.current_delay:.1f}秒")
    ...
    def process_all_files(self):
        ...
        for i, file_name in enumerate(excel_files, 1):
            ...
            # 文件间延迟
            if i < total_files:
                delay = 10 + random.uniform(0, 5)  # 文件间延迟10-15秒
...
def main():
    ...
    processor = ConservativeBatchProcessor()

...
class RedeployedBatchProcessor:
    def __init__(self):
        ...
        # 重新部署后的延迟配置
        self.base_delay = 3.0  # 基础延迟（秒）
        self.max_delay = 15.0  # 最大延迟（秒）
        self.delay_increment = 1.0  # 延迟递增
        self.current_delay = self.base_delay
        
        # 错误统计
        self.error_count = 0
        self.success_count = 0
        self.max_errors = 8  # 最大错误次数
        
        print(f"🔄 EdgeTTS 重新部署批量处理器启动")
        print(f"📁 输入目录: {self.input_dir}")
        print(f"📁 输出目录: {self.output_dir}")
        print(f"🌐 API服务: {len(self.tts_urls)} 个")
        print(f"⏱️ 基础延迟: {self.base_delay}秒 (重新部署模式)")
    ...
    def process_all_files(self):
        ...
        for i, file_name in enumerate(excel_files, 1):
            ...
            # 文件间延迟
            if i < total_files:
                delay = 5 + random.uniform(0, 3)  # 文件间延迟5-8秒
...
def main():
    """主函数"""
    print("🔄 EdgeTTS 重新部署批量处理器")
    print("=" * 60)
    print("🔧 特性:")
    print("   ✅ 使用重新部署的 TTS 服务")
    print("   ✅ 保留原有配置参数")
    print("   ✅ 优化的延迟策略")
    print("   ✅ 多 API 负载均衡")
    print("   ✅ 错误恢复和统计")
    ...
    processor = RedeployedBatchProcessor()
